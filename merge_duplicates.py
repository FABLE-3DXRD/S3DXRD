...
help = """ Script to merge duplicate peaks and eventually maybe Friedel pairs

Can be used to index from a full sinogram as it reduces to remove duplicates
"""
...
def findpairs_2d( cf, dangle, omestep, fac = 1.5, pairs=False):
    """
    ...
    fac = increase angle tol by this
    pairs : overlaps -hkl with hkl or not (clustering)
    """
    ...
    iz = np.digitize( az, zedges) - 1
    iy = np.digitize( ay, yedges) - 1
    H = H.astype( np.int )
    NZ, NY = H.shape
    ...
    ucf = cf.copy()
    ucf.filter( isolated )
    assert ucf.ncols == ucf.bigarray.shape[0]
    newrows = np.zeros( (ucf.ncols, len(clusters)), np.float )
    for i,cl in enumerate(clusters):
        newrows[:,i] = cf.bigarray[ :, cl ].mean(axis=1)
    ucf.bigarray = np.concatenate( ( ucf.bigarray, newrows ), axis=1)
    ucf.nrows = ucf.bigarray.shape[1]
    ucf.set_attributes()        
    if __debug__:
        print( "Time clustering",time.time()-start, ctotal, cf.nrows )
    return ucf
...
def clusterpeaks( cf, pks, d_ds ):
    """
    cf  = columnfile
    pks = selection of peaks to look at
    d_ds = range to consider equivalent
    ...
    # print(ds)
    inds = [np.take(pks,order)[ids==i] for i in range(nclusters) ]
    return nclusters, inds
